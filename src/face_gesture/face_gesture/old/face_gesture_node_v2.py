...
from geometry_msgs.msg import Point, Twist  # Twist 메시지 추가
...
class UltraLightMultiTracker(Node):
    def __init__(self):
        ...
        # [변경/추가] 로봇 모빌리티 제어용 /cmd_vel 퍼블리셔 등록
        ...
        # [추가] 로봇이 현재 이동 중인지 상태를 추적하기 위한 플래그
        ...
        while self.is_running and rclpy.ok():
            ...
            if face_results.multi_face_landmarks:
                for face_landmarks in face_results.multi_face_landmarks:
                    ...
                    # [변경] 실시간 입 벌림 감지 제어 (전진 및 정지)
                    if is_mouth_open:
                        # 입을 벌리고 있는 상태 -> 실시간 0.3 전진 속도 발행
                        self.send_speed_cmd(linear_x=0.3, angular_z=0.0)
                        self.robot_moving = True
                        self.active_gesture_msg = "Mouth Open: Robot Moving Forward (0.3)"
                    else:
                        # 입을 닫은 상태이고 로봇이 움직이고 있었다면 -> 정지 속도 발행
                        if self.robot_moving:
                            self.send_speed_cmd(linear_x=0.0, angular_z=0.0)
                            self.robot_moving = False
                            self.active_gesture_msg = "Mouth Closed: Robot Stopped"

                    # 기존의 타이머 기반 제스처 감지 함수 (WINK 및 TURN 등은 필요시 유지)
                    ...
                    # MOUTH_OPEN 관련 기존 타이머 로직 제거 후 다른 안면 제스처 상태 처리
                    if self.current_state == "IDLE":
                        ...
                        if triggered_gesture:
                            ...
                            if triggered_gesture in ["WINK_LEFT", "WINK_RIGHT"]:
                    ...
                    # 다른 고정형 제스처가 실행 중인 상태 UI 보강
                    if self.current_state != "IDLE":
                        display_text = f"LOCKED: {self.current_state} | {self.active_gesture_msg}"
            ...
            cv2.putText(display, f"FPS: {fps:.1f} | Hand & Face Control", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            cv2.imshow('Ultra Light Tracker', display)
            if cv2.waitKey(1) & 0xFF == ord('q'): break
            time.sleep(max(0, (1.0 / 30.0) - (time.time() - start_time)))

    def destroy_node(self):
        # 안전을 위해 노드 종료 시 로봇 정지 명령 한번 더 송신
